...
Token* readIdentKeyword(void) {
	...
	if (count > 15) error(ERR_IDENTTOOLONG, lineNo, colNo);
...
Token* readNumber(void) {
  // TODO
	...
	while (charCodes[currentChar] == CHAR_DIGIT || 
		...
		str[tokenLength++] = currentChar;
		if (charCodes[currentChar] == CHAR_PERIOD) isFloat = 1;
		readChar();
	//	printf("%d  ", charCodes[currentChar]);
	}
	str[tokenLength] = '\0';
	...
	if (charCodes[currentChar] == CHAR_UNKNOWN || 
		...
		if (isFloat == 1) {
			token = makeToken(TK_FLOAT, line, column);
			strcpy(token->string, str);
			isFloat = 0;
		}
		else {
