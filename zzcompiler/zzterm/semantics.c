...
void checkFreshIdent(char *name) {			// --- Check invalid name ---
...
Object* checkDeclaredLValueIdent2(char* name) {
	Object* obj = lookupObject(name);
	if (obj == NULL){
		error(ERR_UNDECLARED_VARIABLE,currentToken->lineNo, currentToken->colNo);
	}
...
void checkBasicType(Type* type) {
	if ((type != NULL) && ((type->typeClass == TP_INT) || (type->typeClass == TP_CHAR) || (type->typeClass == TP_STRING) || (type->typeClass == TP_FLOAT))){
		return;
	}
	else {
		error(ERR_TYPE_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
