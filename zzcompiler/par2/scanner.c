#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>

#include "reader.c"
#include "charcode.c"
#include "token.c"
#include "error.c"
#include "scanner.h"


extern int lineNo;
extern int colNo;
extern int currentChar;

extern CharCode charCodes[];


void skipBlank() {
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_SPACE))
    readChar();
}

void skipComment() {
  int state = 0;
  while ((currentChar != EOF) && (state < 2)) {
    switch (charCodes[currentChar]) {
      case CHAR_TIMES:
        state = 1;
        break;
      case CHAR_RPAR:
        if (state == 1) state = 2;
        else state = 0;
        break;
      default:
        state = 0;
    }
    readChar();
  }
  if (state != 2) 
    error(ERR_ENDOFCOMMENT, lineNo, colNo);
}

Token* readIdentKeyword(void) {
  Token *token = makeToken(TK_NONE, lineNo, colNo);
  int count = 1;

  token->string[0] = (char)currentChar;
  readChar();

  while ((currentChar != EOF) && 
	 ((charCodes[currentChar] == CHAR_LETTER) || (charCodes[currentChar] == CHAR_DIGIT))) {
    if (count <= MAX_IDENT_LEN) token->string[count++] = (char)currentChar;
    readChar();
  }

  if (count > MAX_IDENT_LEN) {
    error(ERR_IDENTTOOLONG, token->lineNo, token->colNo);
    return token;
  }

  token->string[count] = '\0';
  token->tokenType = checkKeyword(token->string);

  if (token->tokenType == TK_NONE)
    token->tokenType = TK_IDENT;

  return token;
}

Token* readNumber(int check_period) {

  if (check_period == 0){
    Token *token = makeToken(TK_NUMBER, lineNo, colNo);
    int count = 0;

    int check = 0; // tu them - check xuat hien period

    while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)) {
      
      // tu them
      if (charCodes[currentChar] == CHAR_PERIOD) {
        token->tokenType = TK_FLOAT;
        check++;
      } //

      if (check > 1){
        error(ERR_INVALIDFLOAT, token->lineNo, token->colNo);
      }


      token->string[count++] = (char)currentChar;
      readChar();
    }

    token->string[count] = '\0';
    token->value = atoi(token->string);
    return token;


  }else{
    Token *token = makeToken(TK_FLOAT, lineNo, colNo);
    int count = 0;
    token->string[count++] = '.';

    int check = 1; // tu them - check xuat hien period

    while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)) {
      
      // tu them
      if (charCodes[currentChar] == CHAR_PERIOD) {
        check++;
      } //

      if (check > 1){
        printf(" !!!!! \n");
        exit(0);
      }


      token->string[count++] = (char)currentChar;
      readChar();
    }

    token->string[count] = '\0';
    token->value = atoi(token->string);
    return token;
  }
  
}

Token* readConstString(){
	Token *token;
	token = makeToken(TK_STRING, lineNo, colNo);
	int i = 0;

  int string_length_max = 100000; // coi max la 10000 ki tu

	while(charCodes[readChar()] != CHAR_DOUBLEQUOTE){
		token->string[i++] = currentChar;
		if(i == string_length_max || currentChar == '\n'){
			error(ERR_INVALIDSTRING, token->lineNo, token->colNo);
		}
	}
	token->string[i] = '\0';
	readChar();
	return token;
}

Token* readConstChar(void) {
  Token *token = makeToken(TK_CHAR, lineNo, colNo);

  readChar();
  if (currentChar == EOF) {
    token->tokenType = TK_NONE;
    error(ERR_INVALIDCHARCONSTANT, token->lineNo, token->colNo);
    return token;
  }
    
  token->string[0] = currentChar;
  token->string[1] = '\0';

  readChar();
  if (currentChar == EOF) {
    token->tokenType = TK_NONE;
    error(ERR_INVALIDCHARCONSTANT, token->lineNo, token->colNo);
    return token;
  }

  if (charCodes[currentChar] == CHAR_SINGLEQUOTE) {
    readChar();
    return token;
  } else {
    token->tokenType = TK_NONE;
    error(ERR_INVALIDCHARCONSTANT, token->lineNo, token->colNo);
    return token;
  }
}

Token* getToken(void) {
  Token *token;

  if (currentChar == EOF) {
    return makeToken(TK_EOF, lineNo, colNo);
  }

  switch (charCodes[currentChar]) {
    case CHAR_SPACE: skipBlank(); return getToken();

    case CHAR_LETTER: return readIdentKeyword();

    case CHAR_DIGIT: return readNumber(0);

    case CHAR_PLUS:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_ASSIGN_PLUS, lineNo, colNo);
      } else return makeToken(SB_PLUS, lineNo, colNo);

    case CHAR_MINUS:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_ASSIGN_MINUS, lineNo, colNo);
      } else return makeToken(SB_MINUS, lineNo, colNo);

    case CHAR_TIMES:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_ASSIGN_TIMES, lineNo, colNo);
      } else return makeToken(SB_TIMES, lineNo, colNo);

    case CHAR_MOD:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_ASSIGN_MOD, lineNo, colNo);
      } else return makeToken(SB_MOD, lineNo, colNo);

    case CHAR_SLASH:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_ASSIGN_SLASH, lineNo, colNo);
      } else return makeToken(SB_SLASH, lineNo, colNo);

    case CHAR_LT:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_LE, lineNo, colNo);
      } else return makeToken(SB_LT, lineNo, colNo);

    case CHAR_GT:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_GE, lineNo, colNo);
      } else return makeToken(SB_GT, lineNo, colNo);

    case CHAR_EQ: 
      token = makeToken(SB_EQ, lineNo, colNo);
      readChar(); 
      return token;

    case CHAR_EXCLAIMATION:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_NEQ, lineNo, colNo);
      } else {
        token = makeToken(TK_NONE, lineNo, colNo);
        error(ERR_INVALIDSYMBOL, lineNo, colNo);
        return token;
      }

    case CHAR_COMMA:
      token = makeToken(SB_COMMA, lineNo, colNo);
      readChar(); 
      return token;

    case CHAR_PERIOD:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_RPAR)) {
        readChar();
        return makeToken(SB_RSEL, lineNo, colNo);
      } else if((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT)){
        colNo --;
        return readNumber(1);
      } else {
        return makeToken(SB_PERIOD, lineNo, colNo);
      }

    case CHAR_SEMICOLON:
      token = makeToken(SB_SEMICOLON, lineNo, colNo);
      readChar(); 
      return token;

    case CHAR_COLON:
      readChar();
      if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) {
        readChar();
        return makeToken(SB_ASSIGN, lineNo, colNo);
      } else return makeToken(SB_COLON, lineNo, colNo);

    case CHAR_SINGLEQUOTE: return readConstChar();

    case CHAR_DOUBLEQUOTE: return readConstString();

    case CHAR_LPAR:
      readChar();

      if (currentChar == EOF) 
        return makeToken(SB_LPAR, lineNo, colNo);

      switch (charCodes[currentChar]) {
        case CHAR_PERIOD:
          readChar();
          return makeToken(SB_LSEL, lineNo, colNo);
        case CHAR_TIMES:
          readChar();
          skipComment();
          return getToken();
        default:
          return makeToken(SB_LPAR, lineNo, colNo);
        }

    case CHAR_RPAR:
      token = makeToken(SB_RPAR, lineNo, colNo);
      readChar(); 
      return token;

    default:
      token = makeToken(TK_NONE, lineNo, colNo);
      error(ERR_INVALIDSYMBOL, lineNo, colNo);
      readChar(); 
      return token;

  }
}

Token* getValidToken(void) {
  Token *token = getToken();
  while (token->tokenType == TK_NONE) {
    free(token);
    token = getToken();
  }
  return token;
}


void printToken(Token *token) {

  // printf("%2d-%2d : ", token->lineNo, token->colNo);
  if (token->lineNo < 10){
    printf("0%d-", token->lineNo);
  }else{
    printf("%d-", token->lineNo);
  }

  if (token->colNo < 10){
    printf("0%d : ", token->colNo);
  }else{
    printf("%d : ", token->colNo);
  } 

  switch (token->tokenType) {
  case TK_NONE:   printf("TK_NONE\n"); break;
  case TK_IDENT:  printf("TK_IDENT(%s)\n", token->string); break;
  case TK_NUMBER: printf("TK_NUMBER(%s)\n", token->string); break;
  case TK_FLOAT:  printf("TK_FLOAT(%s)\n", token->string); break;
  case TK_CHAR:   printf("TK_CHAR(\'%s\')\n", token->string); break;
  case TK_EOF:    printf("TK_EOF\n"); break;

  case KW_PROGRAM:    printf("KW_PROGRAM\n"); break;
  case KW_CONST:      printf("KW_CONST\n"); break;
  case KW_TYPE:       printf("KW_TYPE\n"); break;
  case KW_VAR:        printf("KW_VAR\n"); break;
  case KW_INTEGER:    printf("KW_INTEGER\n"); break;
  case KW_FLOAT:      printf("KW_FLOAT\n"); break;
  case KW_CHAR:       printf("KW_CHAR\n"); break;
  case KW_ARRAY:      printf("KW_ARRAY\n"); break;
  case KW_OF:         printf("KW_OF\n"); break;
  case KW_FUNCTION:   printf("KW_FUNCTION\n"); break;
  case KW_PROCEDURE:  printf("KW_PROCEDURE\n"); break;
  case KW_BEGIN:      printf("KW_BEGIN\n"); break;
  case KW_END:        printf("KW_END\n"); break;
  case KW_CALL:       printf("KW_CALL\n"); break;
  case KW_IF:         printf("KW_IF\n"); break;
  case KW_THEN:       printf("KW_THEN\n"); break;
  case KW_ELSE:       printf("KW_ELSE\n"); break;
  case KW_WHILE:      printf("KW_WHILE\n"); break;
  case KW_DO:         printf("KW_DO\n"); break;
  case KW_FOR:        printf("KW_FOR\n"); break;
  case KW_TO:         printf("KW_TO\n"); break;

  case SB_SEMICOLON:    printf("SB_SEMICOLON\n"); break;
  case SB_COLON:        printf("SB_COLON\n"); break;
  case SB_PERIOD:       printf("SB_PERIOD\n"); break;
  case SB_COMMA:        printf("SB_COMMA\n"); break;
  case SB_ASSIGN:       printf("SB_ASSIGN\n"); break;
  case SB_ASSIGN_PLUS:  printf("SB_ASSIGN_PLUS\n"); break;
  case SB_ASSIGN_MINUS: printf("SB_ASSIGN_MINUS\n"); break;
  case SB_ASSIGN_TIMES: printf("SB_ASSIGN_TIMES\n"); break;
  case SB_ASSIGN_SLASH: printf("SB_ASSIGN_SLASH\n"); break;
  case SB_ASSIGN_MOD:   printf("SB_ASSIGN_MOD\n"); break;

  case SB_EQ:           printf("SB_EQ\n"); break;
  case SB_NEQ:          printf("SB_NEQ\n"); break;
  case SB_LT:           printf("SB_LT\n"); break;
  case SB_LE:           printf("SB_LE\n"); break;
  case SB_GT:           printf("SB_GT\n"); break;
  case SB_GE:           printf("SB_GE\n"); break;
  case SB_PLUS:         printf("SB_PLUS\n"); break;
  case SB_MINUS:        printf("SB_MINUS\n"); break;
  case SB_TIMES:        printf("SB_TIMES\n"); break;
  case SB_MOD:          printf("SB_MOD\n"); break;
  case SB_SLASH:        printf("SB_SLASH\n"); break;
  case SB_LPAR:         printf("SB_LPAR\n"); break;
  case SB_RPAR:         printf("SB_RPAR\n"); break;
  case SB_LSEL:         printf("SB_LSEL\n"); break;
  case SB_RSEL:         printf("SB_RSEL\n"); break;
  }
}

