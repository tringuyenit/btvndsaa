...
Token* readNumber(int check_period) {
	...
	// while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD)) {
	// 	if(charCodes[currentChar] == CHAR_PERIOD && countForPeriod != 0){
	// 		error(ERR_INVALID_FLOAT_VALUE, token->lineNo, token->colNo);
	// 	}
	// 	if(charCodes[currentChar] == CHAR_PERIOD ){
	// 		if(charCodes[readChar()] != CHAR_DIGIT){
