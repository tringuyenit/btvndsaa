...
Type* duplicateType(Type* type) {			// --- Make the same type, prevent process on root type ---
...
ConstantValue* duplicateConstantValue(ConstantValue* v) {					// --- Make the same constant, prevent process on root constant ---
	ConstantValue* value = (ConstantValue*) malloc(sizeof(ConstantValue));
	value->type = v->type;
	if (v->type == TP_INT)
...
void initSymTab(void) {
	...
	obj = createProcedureObject("WRITELN");					// --- No arguments ---
	addObject(&(symtab->globalObjectList), obj);
...
void enterBlock(Scope* scope) {
	symtab->currentScope = scope;		// --- Update value for currentScope ---
}

void exitBlock(void) {
	symtab->currentScope = symtab->currentScope->outer;			// --- Change block to outer ---
}

void declareObject(Object* obj) {								// --- Declare an object into currentBlock ---
